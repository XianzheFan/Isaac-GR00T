"""
Hybrid sparse+dense clip variant of train_switch_head_gemini.py.

Instead of using the most recent `clip_len` consecutive frames (2s window),
this version uses a hybrid sampling strategy:
  - Far zone:  frames [-200, -20), every 20 frames -> 9 frames covering 18s
  - Near zone: frames [-20,   0],  every 2 frames  -> 11 frames covering 2s
  - Total: 20 frames covering the full 20s Gemini window

This matches the Gemini VLM teacher's 20s observation window while preserving
fine-grained detail in the most recent 2s for capturing rapid changes.

Phase 1 - collect:
  Run SimplerEnv rollouts with the GR00T policy, query Gemini for dense value
  scores, and save hybrid-sampled video clips as .npz.
...
Usage
-----
  # 1) Collect labeled data with hybrid clips (needs GOOGLE_API_KEY set)
  python train_switch_head_gemini_sparse_clip.py collect \
      --model_path /path/to/groot/checkpoint \
      --robot_type google \
      --output_dir data/simpler_env_switch_labels_sparse \
      --far_history 180 --far_stride 20 --near_history 20 --near_stride 2 \
      --num_trials_per_task 20

  # 2) Train standalone switch head (single GPU)
  python train_switch_head_gemini_sparse_clip.py train \
      --data_dir data/simpler_env_switch_labels_sparse \
      --output_dir checkpoints/switch_head_sparse \
      --epochs 30

  # 2b) Multi-GPU training via torchrun (e.g. 8 GPUs)
  torchrun --nproc_per_node=8 train_switch_head_gemini_sparse_clip.py train \
      --data_dir data/simpler_env_switch_labels_sparse \
      --output_dir checkpoints/switch_head_sparse \
      --batch_size 8 \
      --epochs 30
  ...
  python train_switch_head_gemini_sparse_clip.py export \
      --data_dir data/simpler_env_switch_labels_sparse \
    ...
    for task_name, trial_idx in tqdm.tqdm(all_work, desc="rollouts"):
        ...
        for t in range(args.max_episode_steps):
            ...
            try:
                ...
                if not action_plan:
                    ...
                    # Extract hybrid sparse+dense video clip (total 20 frames):
                    #   Far zone:  frames [-200, -20), every 20 frames -> 9 frames (18s)
                    #   Near zone: frames [-20,   0],  every 2 frames  -> 11 frames (2s)
                    # This matches Gemini's 20s window while preserving recent detail.
                    far_history = args.far_history    # 180
                    far_stride = args.far_stride      # 20
                    near_history = args.near_history   # 20
                    near_stride = args.near_stride     # 2
                    total_history = far_history + near_history  # 200

                    all_frames = clean_images[-total_history:]

                    # Far zone: from the older portion, sample sparsely
                    far_portion = all_frames[:max(len(all_frames) - near_history, 0)]
                    far_clip = list(far_portion[far_stride - 1::far_stride])

                    # Near zone: from the most recent portion, sample densely
                    near_portion = all_frames[-min(near_history, len(all_frames)):]
                    near_clip = list(near_portion[near_stride - 1::near_stride])

                    img_clip = far_clip + near_clip
                    clip_len = args.clip_len  # expected total: 20
                    # Trim or pad to exactly clip_len frames
                    img_clip = img_clip[-clip_len:]
        ...
        for step_i, rec in enumerate(replan_records):
            ...
            np.savez_compressed(
                ...
                far_history=np.int32(args.far_history),
                far_stride=np.int32(args.far_stride),
                near_history=np.int32(args.near_history),
                near_stride=np.int32(args.near_stride),
...
class SwitchLabelDataset(Dataset):
    ...
    def __init__(self, data_dir: str, use_clip: bool = True, use_soft_label: bool = False):
        ...
        if self.use_clip:
            ...
            far_h = int(first.get("far_history", 0))
            far_s = int(first.get("far_stride", 1))
            near_h = int(first.get("near_history", 0))
            near_s = int(first.get("near_stride", 1))
            coverage_sec = (far_h + near_h) / ROLLOUT_FPS
            logging.info(
                f"  Using hybrid temporal clips (clip_len={self.clip_len}, "
                f"far={far_h}/{far_s}, near={near_h}/{near_s}, "
                f"coverage={coverage_sec:.1f}s)"
            )
        else:
            self.clip_len = 1
...
def main():
    ...
    p_collect.add_argument("--clip_len", type=int, default=20,
                           help="Total number of frames in hybrid clip (default 20)")
    p_collect.add_argument("--far_history", type=int, default=180,
                           help="Number of frames in far (sparse) zone (default 180 = 18s)")
    p_collect.add_argument("--far_stride", type=int, default=20,
                           help="Sampling stride in far zone (default 20 -> 9 frames)")
    p_collect.add_argument("--near_history", type=int, default=20,
                           help="Number of frames in near (dense) zone (default 20 = 2s)")
    p_collect.add_argument("--near_stride", type=int, default=2,
                           help="Sampling stride in near zone (default 2 -> 11 frames)")
