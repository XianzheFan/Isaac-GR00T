...
RESCUE_CRITICAL_THRESHOLD = 0.50  # critical_score > this → rescue (action-critical phase)
RESCUE_SCORE_RISE = 0.15          # score surge >= this between consecutive queries → rescue
...
def _query_gemini_value(frames: list, task_description: str, step_idx: int,
    ...
    try:
        ...
        prompt = (
            f'You are scoring how much the robot\'s NEXT few actions matter for the task: '
            f'"{task_description}".\n\n'
            f'Think of it this way: if you could slightly perturb the robot\'s next action, '
            f'how much would the outcome change? A high score means a small action difference '
            f'NOW would be the difference between success and failure.\n\n'
            f'CRITICAL RULE: You are NOT scoring how well the task is going. '
            f'A robot that is failing miserably but has already dropped the object gets a LOW '
            f'score (nothing it does now matters). A robot that is doing great but is about to '
            f'make contact with a fragile object gets a HIGH score (precision matters right now).\n\n'
            f'Score 0.80 - 1.00 (Action-critical moment — precision required NOW):\n'
            f'- Gripper is within ~2cm of the target object and closing\n'
            f'- Object is being lifted but grasp is not yet stable\n'
            f'- Robot is navigating a tight gap or aligning for insertion\n'
            f'- Object is in gripper but shifting/rotating during transport\n'
            f'- Any moment where 1mm or 1-degree error changes the outcome\n\n'
            f'Score 0.50 - 0.80 (Moderately sensitive):\n'
            f'- Robot is approaching the object and is ~2-8cm away\n'
            f'- Gripper is open and descending toward the target\n'
            f'- Object is stably grasped but being moved toward a goal position\n'
            f'- Drawer/door is being pushed but not yet at the target position\n\n'
            f'Score 0.20 - 0.50 (Low sensitivity):\n'
            f'- Robot arm is far from any object, moving in the right general direction\n'
            f'- Early phase of reaching with plenty of room to correct\n'
            f'- Object is stably held and far from any obstacle\n\n'
            f'Score 0.00 - 0.20 (Not sensitive — actions barely matter):\n'
            f'- Robot arm is moving aimlessly, nowhere near the target\n'
            f'- Task has already clearly failed (object dropped, knocked away) and '
            f'the situation is unrecoverable\n'
            f'- Task is already complete and the robot is just holding still\n'
            f'- Robot is in its initial pose, not yet moving\n\n'
            f'FOCUS on the LAST few frames of the video. The history is context only.\n\n'
            f'Output strictly in JSON array format:\n'
            f'[{{"reasoning": "Brief explanation of what the robot is doing NOW and why '
            f'precision matters or does not.", "critical_score": 0.75, '
            f'"is_critical": true, '
            f'"phase_type": "approach/contact/grasp/transport/placement/idle/failed", '
            f'"failure_risk": "low/medium/high", '
            f'"recoverability": "easy/moderate/hard", '
            f'"status": "Approaching target"}}]'
        )
        ...
        if result:
            score = result[0].get("critical_score")
            if score is not None:
                with lock:
                    score_history.append((step_idx, float(score)))
                logging.info(
                    f"[Gemini Critical] frame={step_idx} critical_score={score:.2f} "
                    f"is_critical={result[0].get('is_critical')} "
                    f"phase_type={result[0].get('phase_type')} "
...
def _check_rescue_needed(score_history: list, lock: threading.Lock) -> bool:
    """Return True if rescue should be triggered based on the Gemini critical score.

    High critical_score means the current action is sensitive / precision-critical,
    so we trigger rescue when the score exceeds the threshold.
    """
    with lock:
    ...
    # Condition 1: absolute high score (action-critical phase)
    if latest_score > RESCUE_CRITICAL_THRESHOLD:
        logging.info(f"[Rescue] Triggered: critical_score {latest_score:.2f} > {RESCUE_CRITICAL_THRESHOLD}")
        return True

    # Condition 2: score surge (rapidly entering critical phase)
    prev_score = None
    for frame_idx, score in reversed(sorted_scores[:-1]):
        if latest_frame - frame_idx >= GEMINI_QUERY_INTERVAL_FRAMES:
            prev_score = score
            break
    if prev_score is not None and (latest_score - prev_score) >= RESCUE_SCORE_RISE:
        logging.info(
            f"[Rescue] Triggered: score surged {prev_score:.2f} -> {latest_score:.2f} "
            f"(rise={latest_score - prev_score:.2f} >= {RESCUE_SCORE_RISE})"
        )
        return True
...
def collect(args):
    ...
    for task_name, trial_idx in tqdm.tqdm(all_work, desc="rollouts"):
        ...
        for rec in replan_records:
            ...
            # Condition 1: absolute high score
            if latest_s > RESCUE_CRITICAL_THRESHOLD:
                should_rescue = True
            # Condition 2: score surge
            if not should_rescue:
                ...
                for f, s in reversed(scores_up_to[:-1]):
                    if latest_f - f >= GEMINI_QUERY_INTERVAL_FRAMES:
                        prev_s = s
                        break
                if prev_s is not None and (latest_s - prev_s) >= RESCUE_SCORE_RISE:
                    should_rescue = True
...
class SwitchLabelDataset(Dataset):
    ...
    def __init__(self, data_dir: str, use_clip: bool = True, use_soft_label: bool = False):
        ...
        if self.use_soft_label:
            logging.info("  Using soft labels (gemini_score as critical_score directly)")
    ...
    def __getitem__(self, idx):
        ...
        if self.use_soft_label and "gemini_score" in data:
            # Critical score: high score = critical/needs intervention, use directly
            label = torch.tensor(float(data["gemini_score"]), dtype=torch.float32)
        else:
...
def train(args):
    ...
    if use_soft_label:
        if is_main:
            logging.info("Using MSE loss for soft-label regression (target = critical_score directly)")
