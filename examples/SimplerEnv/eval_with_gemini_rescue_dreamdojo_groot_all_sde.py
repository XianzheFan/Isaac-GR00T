...
**ALL inference uses SDE (Euler-Maruyama) sampling** — both normal policy
rollout and rescue candidate generation use stochastic diffusion.
...
Usage:
    python examples/SimplerEnv/eval_with_gemini_rescue_dreamdojo_groot_all_sde.py \
...
def _rescue_select_action(
    ...
    # SDE head is already active — sample multiple action chunks
    action_chunks = []
...
class Args:
    ...
    """SDE diffusion noise strength (used for all inference)."""
    num_inference_timesteps: int | None = None
    """Number of denoising steps for SDE (None uses model default)."""
    action_horizon: int = 1
    """Number of actions to execute per normal policy query (1 = query every step)."""
...
def eval_simpler_env(args: Args) -> None:
    ...
    if not args.video_out_path:
        args.video_out_path = f"data/simpler_env_rescue_all_sde_{args.robot_type}"
    ...
    # Build SDE action head and replace the default ODE head for ALL inference
    ...
    sde_action_head.load_state_dict(base_policy.model.action_head.state_dict())
    sde_action_head.to(device=args.device, dtype=torch.bfloat16)
    sde_action_head.eval()

    # Replace action head with SDE for all inference
    base_policy.model.action_head = sde_action_head
    logging.info("GR00T policy loaded (SDE for all inference).")
                ...
                try:
                    ...
                    if not action_plan:
                        ...
                        if rescue:
                            ...
                            # Normal GR00T SDE inference (stochastic)
                            groot_obs = build_groot_obs_fn(obs)
                            action_dict, _ = groot_policy.get_action(groot_obs)
